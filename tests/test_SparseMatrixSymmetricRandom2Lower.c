...
int main(int argc, char **argv) {
    ...
    long n, nz, i, j, k, t, val, **a ;

    printf("Test SparseMatrixSymmetricRandom2Lower: ");
    n = 79 ; /* n by n real matrix A with stored nonzero diagonal
                i = j + k, where k >= 1, and a stored main diagonal 
                i = j. The diagonal i = j - k is not stored. */
    k = 5 ; 
    A.m = n;
    A.n = n;
    A.NrNzElts = 2*n-k ; 
    ...
    /* Fill dense and sparse matrices with nonzeros below main diagonal */
    t = 0;
    for (i=k; i<n; i++){
        j = i - k ;
        A.i[t] = i ;
    ...
    /* Fill dense and sparse matrices with diagonal nonzeros */
    for (i=0; i<n; i++){
        A.i[t] = i ;
        A.j[t] = i ;
        a[i][i] = i*n+i ;
